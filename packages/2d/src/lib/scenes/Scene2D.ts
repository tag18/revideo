  ...
  public override getMediaAssets(): Array<AssetInfo> {
    ...
    const playingAudios = allAudios
      .filter(audio => {
        ...
        // Filter out audio that has finished playing (currentTime reached duration)
        // Only filter if currentTime is actually moving (not stuck at 0)
        if (currentTime > 0 && currentTime >= duration - 0.01) {
          return false;
        }
        
        // Keep all other playing audio
        return true;
      });
