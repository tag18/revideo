...
export class EditableTimeEvents implements TimeEvents {
  ...
  public register(name: string, initialTime: number): number {
    ...
    if (!event) {
      ...
      // Always update initialTime to match the current code position
      if (newEvent.initialTime !== initialTime) {
        newEvent.initialTime = initialTime;
        changed = true;
      }

      // When preserveTiming is true, recalculate offset from targetTime
      // This keeps the event at its dragged position even if code execution point changes
      ...
      // When preserveTiming is false, recalculate targetTime from offset
  ...
  private handleRecalculated = () => {
    ...
    if (
      ...
      const dataToSave = [...this.registeredEvents.values()].map(
        event => ({
          name: event.name,
      ...
      this.previousReference = dataToSave;
